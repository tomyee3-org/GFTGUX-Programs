...
def _timestamp_fname(now, prefix="gw_inspiral"):
    ...
    See driver_gw._timestamp_fname for the full rationale (taking `now` as
    an explicit argument so filename and metadata timestamps always agree;
    UTC so PNG and CSV filenames from the same run sort and compare
    directly; microsecond resolution reduces, but does not eliminate,
    collisions -- _publish_atomically() below is what actually guarantees
    no silent overwrite).
    """
...
def _create_private_temp_file(directory, ext="png"):
    ...
    `directory` to render the PNG into before publication, and return its
    open file descriptor together with its path.

    See driver_gw._create_private_temp_file for the full rationale;
...
def _create_mode_probe_file(directory):
    """Create a uniquely-named, empty file in `directory`, requesting mode
    0o666, and return its open descriptor and path -- lets the kernel apply
    the process umask atomically during creation, without this program
    reading or mutating that process-wide state itself.

    See driver_gw._create_mode_probe_file for the full rationale;
    duplicated here rather than imported, per this project's existing
    convention.
    """
    for _ in range(1000):
...
def _default_output_file_mode(directory):
    ...
    See driver_gw._default_output_file_mode for the full rationale;
    duplicated here rather than imported, per this project's existing
    convention of not sharing code across the driver_gw/plot_gw split.
    """
...
def _verify_temp_identity(fd, tmp_path):
    ...
    See driver_gw._verify_temp_identity for the full rationale and for
    exactly what this does and does not guarantee; duplicated here rather
...
def _publish_atomically(fd, tmp_path, directory, prefix, now, ext="png", max_attempts=1000):
    ...
    See driver_gw._publish_atomically for the full rationale (os.link()
    creates the public name only once tmp_path already holds the complete
    PNG, retried with a "_1", "_2", ... suffix on a name collision rather
    than overwriting; the atomicity guarantee holds on filesystems with
    ordinary POSIX hard-link semantics, not universally; this is not a
    full crash-durability guarantee). _verify_temp_identity(fd, tmp_path)
    is called immediately before the first os.link() attempt -- see that
...
def _publish_or_cleanup(write_fn, directory, prefix, now):
    ...
    See driver_gw._publish_or_cleanup for the full rationale; duplicated
    here rather than imported, per this project's existing convention of
    not sharing code across the driver_gw/plot_gw split.
...
def plot_inspiral(result, outdir=None,
    ...
    try:
        ...
        # savefig() leaves no PNG behind rather than a zero-byte or
        # partial one.
        if outdir is not None:
            def _do_save(fd, tmp_path):
                # Save through the already-open fd (via os.fdopen(...,
                # closefd=False)) rather than passing tmp_path to
                # savefig() by name -- see
                # driver_gw._create_private_temp_file's docstring for why
                # a path-based reopen here would reintroduce a symlink-
                # substitution race. format="png" stays explicit: a file
                # object's .name is not a real ".png" path Matplotlib can
                # sniff an extension from.
                with os.fdopen(fd, "wb", closefd=False) as handle:
                    fig.savefig(handle, dpi=dpi, bbox_inches="tight", format="png")
                    # fsync the PNG's own file content before it is
                    # published -- Matplotlib's savefig() does not fsync
                    # on its own, and _publish_atomically() only best-
                    # effort-fsyncs the *directory* entry, not file data.
                    handle.flush()
                    os.fsync(fd)
        ...
        # Must run even if savefig() or anything else above raised, so a
        # plotting exception can never leak a live figure into pyplot's
        # registry.
        plt.close(fig)
